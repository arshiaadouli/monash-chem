...
def experiment_detail(request, pk):
    ...
    experiment = Experiment.objects.get(id=pk)          #make sure this is not iterable: set correct permissions!
    supplies = SuppliesTable(Experiment_Chemicals.objects.filter(experiment=pk))
    file_form = AddFileForm(pk=pk)
    files_list = MeasurementTable(Measurement.objects.filter(experiment=pk))
    supplies = Experiment_Chemicals.objects.all()
    context = {
    ...
    # for i in items:
    #     print(i.getExperiments.all()[0].id)

    return render(request, "experiments/experiment.html", context)

@login_required
def experiment_detail_delete(request, path):
    data = request.POST
    id = data.get('id')
    Experiment_Chemicals.objects.filter(id=id).delete()
    print(path)
